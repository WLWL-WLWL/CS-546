...
async function create(name, location, phoneNumber, website, priceRange, cuisines, overallRating, serviceOptions) {
    if (name === undefined || location === undefined || phoneNumber === undefined || website === undefined || priceRange === undefined || cuisines === undefined ||
        overallRating === undefined || serviceOptions === undefined) throw "Field not exisit";
...
async function remove(id) {
    ...
    const deletRes = restaurantCollection.deleteOne({ _id: parseId })

    if (deletRes.deletedCount === 0) throw `Could not delete restaurant with id of ${id}`;
    let res = `${restaurant.name} has been successfully deleted`;
...
async function rename(id, newWebsite) {
    checkId(id);
    if (newWebsite === undefined) throw "Website must be provided";
    if (typeof newWebsite !== 'string') throw "Website must be string";
    if (newWebsite.length == 0 || newWebsite.trim().length == 0) throw "Website cannot be empty";
    if (newWebsite.toLowerCase().startsWith("http://www.") == false || newWebsite.toLowerCase().endsWith(".com") == false || newWebsite.length < 20) throw "Invalid website format";


    let parseId = ObjectId(id);
    const restaurantCollection = await restaurants();
    const restaurant = await this.get(id);
    if (restaurant.website == newWebsite) throw "Same website";
    const renametWeb = restaurantCollection.updateOne({ _id: parseId }, { $set: { website: newWebsite } });
    if (renametWeb.modifiedCount === 0) throw 'Could not update restaurant successfully';
